...
public class NewsController : ControllerBase
{
    ...
    public IActionResult Board()
    {
        ...
        var result = newsList.Select(n => new BoardViewModel()
        {
            ...
            RemainigDays = n.Date.RemainingDays,
    ...
    public IActionResult FindById(Guid id)
    {
        if (ModelState.IsValid)
        {
            ...
            var resultado = new NewsPresentation
            {
                Id = news.Id,
    ...
    public ActionResult Create([FromBody] AddNewsInput news)
    {
        if (ModelState.IsValid)
        {
            var newsToAdd = News.Create(
                ...
                Place.Create(news.City, news.Street, news.Number,
                    news.Reference),
                news.Description);
    ...
    public ActionResult Update([FromBody] EditNewsInput news)
    {
        if (ModelState.IsValid)
        {
            ...
            {
                return NotFound($"The news {newsToUpdate.Id}" + 
                                    "does not exist");
            }
            newsToUpdate.Update(newsToUpdate.Date,
                newsToUpdate.Place, news.Description);
            return new OkObjectResult(
                new { url = $"news/{news.Id}" });
        }
