...
public class FriendController : Controller
{
    ...
    {
        var friend = Friend.Create(model.Name,
             model.Phone, model.Email, model.Password);
        ...
        [FromServices] IHttpClientFactory clientFactory)
    {
        ...
        var mod = client.GetAsync($"api/friends/{text}").Result;
        var model = HttpClientJsonExtensions.GetFromJsonAsync<FriendApiModel>
            (client, $"api/Friends/{text}").Result;
        return View(model);
    }
}
